...
<template>
  <div class="grid grid-cols-2 relative">
    <div class="flex flex-col justify-center mx-auto">
      ...
      <p>
        Avg distance:
        {{ (Number(state.data.avg_dist_from) / 1000).toFixed(2) }}km
      </p>
      <p class="text-center font-semibold">Top 5</p>
      <ul>
        <li
          v-for="(item, i) of state.data.starting_from_top"
          v-bind:key="item.numberoftrips"
        >
    ...
    <div class="flex flex-col align-middle mx-auto">
      ...
      <p>
        Avg distance: {{ (Number(state.data.avg_dist_to) / 1000).toFixed(2) }}km
      </p>
      <p class="text-center font-semibold">Top 5</p>
      <ul>
        <li
          v-for="(item, i) of state.data.ending_to_top"
          v-bind:key="item.numberoftrips"
        >
          {{ i + 1 }}. {{ item.departure_station_name }}:
          {{ item.numberoftrips }}
        </li>
      </ul>
    </div>
    <div class="absolute right-5">
      ...
      >
        <option v-for="s of selections" v-bind:key="s.name" :value="s.name">
          {{ s.name }}
        </option>
      </select>
    </div>
